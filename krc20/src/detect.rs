        ...
        || find(signature_script, &kasplex_header_uppercase.to_vec()).is_some()
    {
        ...
        let _script_result: Result<(), TxScriptError> = parse_script(signature_script)
            .try_for_each(|opcode| {
                ...
                println!("Iter get_data OpCode: {:?}", opcode.get_data());
                ...
                // Actual implementation: pushed data is a script.
                
                if !opcode.is_empty()
                    && opcode.is_push_opcode(){

                    let _result: Result<(), TxScriptError> = parse_script(opcode.get_data())
                    .try_for_each(|inner_opcode| {
                        let inner_opcode: Box<dyn OpCodeImplementation<PopulatedTransaction>> = inner_opcode?;
                        if !inner_opcode.is_empty()
                            && inner_opcode.is_push_opcode()
                        //     && (find(inner_opcode.get_data(), &krc20_beacon.to_vec()).is_some()
                        //         || find(inner_opcode.get_data(), &krc20_uppercase_beacon.to_vec()).is_some())
                        {
                            print!("");
                            print!("Found krc-20 beacon");
                            print!("");
                            if let Some(data) = deserialize(inner_opcode.get_data()) {
                ...
                // Understood implementation: pushed data is JSON
                if !opcode.is_empty()
                    && opcode.is_push_opcode()
...
pub fn observed_tn11() {
    // 0x41 data
    let observed_op_data65: [u8; 65] = [
        104, 156, 143, 162, 42, 151, 189, 142, 191, 189, 50, 199, 200, 143, 148, 101, 96, 46, 229,
        246, 45, 119, 198, 222, 15, 9, 243, 187, 9, 241, 226, 152, 79, 188, 88, 138, 32, 209, 34,
        117, 228, 78, 29, 86, 248, 150, 230, 109, 162, 126, 18, 180, 67, 85, 125, 156, 93, 186,
        211, 226, 126, 93, 248, 197, 1,
    ];

    // 0x4c data or OpPushData2 also supported
    let observed_op_pushdata1: [u8; 91] = [
        32, 138, 214, 99, 52, 105, 85, 129, 55, 75, 214, 225, 186, 91, 113, 13, 54, 86, 144, 215,
        167, 88, 83, 88, 82, 254, 130, 35, 222, 236, 197, 65, 231, 172, 0, 99, 7, 107, 97, 115,
        112, 108, 101, 120, 81, 1, 0, 0, 41, 123, 34, 112, 34, 58, 34, 75, 82, 67, 45, 50, 48, 34,
        44, 34, 111, 112, 34, 58, 34, 109, 105, 110, 116, 34, 44, 34, 116, 105, 99, 107, 34, 58,
        34, 111, 106, 98, 107, 111, 34, 125, 104,
    ];
    // contains { p: "KRC-20", op: "mint", tick: "ojbko", max: None, lim: None, amt: None, dec: None }

    println!("{:x?}", observed_op_data65);
    println!("{:x?}", observed_op_pushdata1);

    detect_krc20(&observed_op_pushdata1);
}
